...
inline size_t Cave::CountNeighbours(size_t row, size_t col) {
    return CheckNeighbour(row, col - 1) + CheckNeighbour(row, col + 1) + CheckNeighbour(row - 1, col) +
           CheckNeighbour(row + 1, col) + CheckNeighbour(row - 1, col - 1) + CheckNeighbour(row - 1, col + 1) +
           CheckNeighbour(row + 1, col - 1) + CheckNeighbour(row + 1, col + 1);
}

void Cave::SetFirstGeneration(size_t live_chance) {
    for (auto& cell : *this) {
        int chance = GenRandCond(0,100);
...
void Cave::Generate(size_t rows, size_t cols, size_t life_chance, const Range& live, const Range& born) {
    if (rows == 0 || cols == 0) {
        return;
    }
    Clear();
...
void Cave::Generate_() {
    ...
    for (size_t row = 0; row != GetRows(); ++row) {
        for (size_t col = 0; col != GetCols(); ++col) {
            ...
            if (At(row, col) == 1 && (count < live_limit_.first || count > live_limit_.second)) {
                tmp[row][col] = 0;
            } else if (At(row, col) == 0 && count >= born_limit_.first && count <= born_limit_.second) {
                tmp[row][col] = 1;
            }
        }
    }
