...
bool SolveRecursive(SolutionMap& solution_map, const WallsMap& walls_map,
                    Indices curr, const Indices& target) {
  auto [i, j] = curr;
  solution_map[i][j].visited = true;
  if (curr == target) {
    return true;
  }
  if (j < walls_map.GetCols() - 1 && !walls_map[i][j].right_wall &&
  ...
  if (i < walls_map.GetRows() - 1 && !walls_map[i][j].bottom_wall &&
      !solution_map[i + 1][j].visited &&
      SolveRecursive(solution_map, walls_map, {i + 1, j}, target)) {
...
inline bool BoundsCheck(const Indices& idx,
                        const WallsMap& walls_map) noexcept {
  return idx.first < walls_map.GetRows() && idx.second < walls_map.GetCols();
}

}  // namespace

std::pair<bool, SolutionMap> Solve(const WallsMap& walls_map,
                                   const Indices& curr, const Indices& target) {
  if (!walls_map.GetRows() || !walls_map.GetCols() ||
      !BoundsCheck(curr, walls_map) || !BoundsCheck(target, walls_map)) {
    return {true, {}};
  }
